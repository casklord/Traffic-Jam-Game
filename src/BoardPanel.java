...
public class BoardPanel{
	...
	public JPanel makeBoardPanel(GameFrame f, Level level) {
        ...
        //Number of moves
        movesLabel = new JLabel("Moves: " + numMoves);
        ...
        //HORIZONTAL
        for (int index = 0; index<level.getRowList().size(); index++) {
        	ArrayList<Vehicle> vrow = level.getRowList().get(index);
        	if (!(vrow.isEmpty())){
        		for (int i = 0; i < vrow.size(); i++) {
            		...
            		if (car.getLength() == 1) {
            			if (index == 2) {
            				car.setIcon(new ImageIcon(getClass().getResource("Exit.png")));
            			}
            			else {
            		...
            		VehicleController mouse = new VehicleController(gameState, f, this);
