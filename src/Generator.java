...
public class Generator {
	 ...
	 */
	public Level generate(int nCars){
		grid = new int[6][6];
		...
		ArrayList<ArrayList<Vehicle>> verV = new ArrayList<ArrayList<Vehicle>>();
		ArrayList<ArrayList<Vehicle>> horV = new ArrayList<ArrayList<Vehicle>>();
		for(Vehicle v : vehicles){
			if(v.getOrientation() == 0){
				verV.get(v.getStartPos()).add(v);
			} else {
				horV.get(v.getStartPos()).add(v);
			}
		}
		Level l = new Level();
		l.setRowList(horV);
